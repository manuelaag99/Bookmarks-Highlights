...
const FormForAddOrUpdate = props => {
    const [inputState, dispatch] = React.useReducer(addOrUpdateIndividualInputReducer, { value: (props.initialValue) || "", isValid: props.isAddPage ? false : true})
    ...
    return (
        <div className={"h-16 mt-3 " + props.classnames}>
            ...
            <input onChange={inputChangeHandler} onBlur={inputBlurHandler} className="block outline-none mt-1 pl-2 h-8 w-95 pr-2" type={props.inputType} placeholder={props.placeholderText} value={inputState.value} />
        </div>
    )
}

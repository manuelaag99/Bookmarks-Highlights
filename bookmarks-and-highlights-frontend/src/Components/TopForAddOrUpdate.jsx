...
export default function TopForAddOrUpdate ({ classnames, form, isAddOrUpdateBtnAbled, isUpdating, route, stateToSend, type, userid }) {
    ...
    <div className="flex flex-wrap md:flex-col flex-row w-full absolute top-0 md:h-1/4 h-16 justify-between">
        ...
        <div className="md:h-6/10 h-full flex flex-wrap flex-row justify-around md:w-full w-6/10">
            ...
            <Button classnames={" bg-var-4 hover:bg-var-4-hovered text-var-1 md:text-dsk-addBandHbtns text-mob-addBandHbtns min-h-2/3 my-auto " + classnames} type={type} form={form} isAbled={isAddOrUpdateBtnAbled} linkRoute={isUpdating ?  "/" + userid + "/bandhs" : "/" + userid + "/myprofile"} buttonText={isUpdating ? "update" : "add"} />

...
export default function FormWithListForAddOrUpdate({ classnames, errorText, field, initialValidity, initialValue, inputType, isBookListOpen, labelText, listOfBooks, onInput, placeholderText, selectTitle, shouldBookListClose, shouldBookListOpen, valueFromList }) {
    const DUMMYOPTIONS = [
        { bookTitle: "The History of Europe", bookId: "85jt95rh4897h5948rj3i" },
    ...
    let newList = DUMMYOPTIONS;
    const [filteredList, setFilteredList] = useState(newList);
    const filterList = () => {
        newList = DUMMYOPTIONS.filter(option => {
            return option.bookTitle.toLowerCase().includes(dynamicValue)
        });
    ...
    return (
        <div className={"h-16 mt-3 " + classnames}>
            ...
            {isBookListOpen && <div className="absolute bg-var-1 text-var-5 w-95 h-fit cursor-pointer shadow-card">
                {listQuery && filteredList.map((book, index) => <p key={index} onClick={clickHandle} className="hover:bg-var-4 hover:text-var-1 duration-100 w-full px-3 py-2 h-fit place-self-center">{book.bookTitle}</p>)}
                <input className="outline-none py-2 px-3 h-fit items-center w-8/10" onChange={filterList} value={dynamicValue} />
                <button className="w-1/10 h-full justify-center items-center px-4 py-2 hover:bg-var-4 hover:text-var-1">Add</button>
            </div>}
        </div>
    );
};
...
export default function FormWithListForAddOrUpdate({ classnames, errorText, existingBooks, field, initialValidity, initialValue, inputType, isAdd, isBookListOpen, labelText, onInput, placeholderText, selectTitle, shouldBookListClose, shouldBookListOpen, userid, valueFromList }) {
    ...
    if (loading) {
        ...
        return (
            <div className={"h-16 mt-3 " + classnames}>
                ...
                <input className="block outline-none mt-1 pl-2 h-8 w-95 pr-2 sm:text-add-or-update-p text-add-or-update-mob" type={inputType} placeholder={placeholderText} value={dynamicValue} onFocus={focusInHandler} onBlur={inputBlurHandler} onChange={inputChangeHandler} />
                {isBookListOpen && <div className="absolute flex flex-col bg-var-1 text-var-5 w-95 h-fit cursor-pointer shadow-card">
                    ...
                    <div className="flex flex-row items-center">
                        <input className="outline-none py-2 px-3 h-fit items-center w-8/10" value={dynamicValue} onChange={inputChangeHandler} />

  ...
  return (
    <div className="flex flex-wrap justify-center mx-auto md:w-full w-8/10">
      <div className="md:absolute fixed w-full h-16 z-20">
        <Link
          className="md:w-1/12 w-1/10 h-full absolute left-0 top-0"
          to={"/" + userid + "/myprofile"}
        >
      ...
      <ProfileTop
        ...
        bio={
          entries.length > 1
            ? entries.length + " photos in this collection"
            : entries.length === 1
            ? "1 photo in this collection"
            : "There are no photos in this collection yet"
        }
      />
      <Options isProfilePage={false} margins={true} rightText="order by: " />
      <CardsSection
        isItByBookTitle={isTitleTheBookTitle}
        isNotBandHsPage={false}
        isBandHsPage={true}
        userid={userid}
        bookid={bookid}
        title={title}
